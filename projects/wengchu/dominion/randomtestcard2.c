...
int main()
{
    ...
    for (x = 0; x <= 50; x++)
    {
        ...
        minionR(choice1, choice2, currentPlayer, &state, handPos, &bonus);
        ...
        if(choice1 == 1 && choice2 == 0 )
        {
            ...
            //test +1 action
            printf("actions = %d, expected = %d\n", state.numActions, testAction + 1); //test introduce bug 1
            assertM("+1 action", state.numActions == testAction + 1);  
            //test +2 coins
            printf("coins= %d, expected = %d\n", state.coins, testCoin + 2); //test introduce bug 2
