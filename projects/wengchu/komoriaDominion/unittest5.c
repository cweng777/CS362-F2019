...
#define TEST_FUNCTION "handleMine"
...
int main()
{
    ...
    handleMine(currentPlayer, choice1, choice2, &stateT, 0);
    ...
    printf("buys = %d, expected = %d\n", stateT.numBuys, testBuy);
    assertM("unchanged buy", stateT.numBuys == testBuy);
    printf("--------------------\n");
    printf("actions = %d, expected = %d\n", stateT.numActions, testAction);
    assertM("unchanged action", stateT.numActions == testAction);
    printf("--------------------\n");
    printf("ID of card gained in hand = %d, expected = %d\n", stateT.hand[currentPlayer][0], gold); //new gold card will be handPos because setup in the discardCard function
    assertM("+ gold card in hand", stateT.hand[currentPlayer][0] == gold);
    printf("--------------------\n");
    ...
    stateT.hand[currentPlayer][choice1] = silver;
    choice2 = province; 
    int testCardInHand = stateT.hand[currentPlayer][0];
    ...
    printf("\n");
    printf("HandPos card ID in hand = %d, expected = %d\n", stateT.hand[currentPlayer][0], testCardInHand); //2 bug(>3 and range too large) (discussion) cause province card can be gained, shouldn't
    assertM("HandPos card in hand should not be changed", stateT.hand[currentPlayer][0] == testCardInHand);
